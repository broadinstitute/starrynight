...
def generate_download_list():
    # Initialize the download list file
    with open(DOWNLOAD_LIST, "w") as f:
        ...
        for cycle in range(1, 4):
            for well in wells:
                for site in sites:
                    ...
                    f.write(f"cp '{s3_file}' {local_dir}\n")
        ...
        f.write(f"cp '{s3_file}' {metadata_dir}\n")


def main():
    print(f"Creating directories and generating download list at {DOWNLOAD_LIST}")
    create_directories()
    generate_download_list()
    print(f"Download list created at {DOWNLOAD_LIST}")
    print(
        "Review the file and run the download command at the end to download all files."
    )
