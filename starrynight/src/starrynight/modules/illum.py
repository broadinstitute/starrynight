...
def create_pcp_illum(
    pipeline: Pipeline,
    load_data_path: Path,
) -> Pipeline:
    load_data_df = pl.read_csv(load_data_path)
    module_counter = 0
...
def illum_pipe_gen(load_data_path: Path, write_path: Path):
    # Default run dir should already be present, otherwise CP raises an error
    with CellProfilerContext(Path(__file__).parent.joinpath("run")) as cpipe:
        cpipe = create_pcp_illum(cpipe, load_data_path)
...
def run_illum_parallel(illum_cppipes_path: Path, jobs: int = 20) -> None:
    illum_cppipes = [path for path in illum_cppipes_path.glob("**/*.cppipe")]
    cellprofiler_core.utilities.java.start_java()
    parallel(illum_cppipes, run_illum, [], jobs)
