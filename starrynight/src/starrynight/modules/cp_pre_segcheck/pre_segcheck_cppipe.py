...
class CPPreSegcheckGenCPPipeModule(StarrynightModule):
    ...
    def _spec() -> str:
        ...
        return SpecContainer(
            ...
            citations=TypeCitations(
                algorithm=[
                    TypeAlgorithmFromCitation(
                        name="Starrynight CP pre-segmentation check generate cppipe module",
                        description="This module generates cppipe files for cp pre-segmentation check module.",
                    )
                ]
            ),
        )
    ...
    def from_config(
        ...
        if spec is None:
            spec = CPPreSegcheckGenCPPipeModule._spec()
            spec.inputs[0].path = (
