...
def create_pipe_gen_cpinvoke(uid: str, spec: SpecContainer) -> Pipeline:
    ...
    uid: str
        Module unique id.
    spec: SpecContainer
        CPPreSegcheckInvokeCPModule specification.
...
class CPPreSegcheckInvokeCPModule(StarrynightModule):
    ...
    def _spec() -> str:
        ...
        return SpecContainer(
            ...
            citations=TypeCitations(
                algorithm=[
                    TypeAlgorithmFromCitation(
                        name="Starrynight CP pre-segmentation check invoke cellprofiler module",
                        description="This module invoke cellprofiler for applying cp pre-segmentation check.",
                    )
                ]
            ),
        )
    ...
    def from_config(
        ...
        if spec is None:
            spec = CPPreSegcheckInvokeCPModule._spec()
            spec.inputs[0].path = (
                list(
            ...
            spec.outputs[0].path = (
                data.workspace_path.joinpath(CP_PRE_SEGCHECK_OUT_PATH_SUFFIX)
                .resolve()
                .__str__()
            )
        pipe = create_pipe_gen_cpinvoke(
            uid=CPPreSegcheckInvokeCPModule.uid(),
            spec=spec,
        )
        uow = create_work_unit_gen_index(out_dir=data.storage_path.joinpath("index"))

        return CPPreSegcheckInvokeCPModule(spec=spec, pipe=pipe, uow=uow)
