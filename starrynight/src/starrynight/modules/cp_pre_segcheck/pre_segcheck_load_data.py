...
class CPPreSegcheckGenLoadDataModule(StarrynightModule):
    ...
    def _spec() -> SpecContainer:
        ...
        return SpecContainer(
            ...
            citations=TypeCitations(
                algorithm=[
                    TypeAlgorithmFromCitation(
                        name="Starrynight CP pre-segmentation check generate loaddata module",
                        description="This module generates load data files for cp pre segcheck module.",
                    )
                ]
            ),
        )
    ...
    def from_config(
        ...
        pipe = create_pipe_gen_load_data(
            uid=CPPreSegcheckGenLoadDataModule.uid(),
            spec=spec,
        )
        uow = create_work_unit_gen_index(out_dir=data.storage_path.joinpath("index"))

        return CPPreSegcheckGenLoadDataModule(spec=spec, pipe=pipe, uow=uow)
