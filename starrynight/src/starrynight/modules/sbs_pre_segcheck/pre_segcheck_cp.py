...
from starrynight.modules.sbs_pre_segchek.constants import (
...
def create_pipe_gen_cpinvoke(uid: str, spec: SpecContainer) -> Pipeline:
    ...
    uid: str
        Module unique id.
    spec: SpecContainer
        SBSPreSegcheckInvokeCPModule specification.
...
class SBSPreSegcheckInvokeCPModule(StarrynightModule):
    ...
    def _spec() -> str:
        ...
        return SpecContainer(
            ...
            citations=TypeCitations(
                algorithm=[
                    TypeAlgorithmFromCitation(
                        name="Starrynight SBS pre-segmentation check invoke cellprofiler module",
                        description="This module invoke cellprofiler for applying sbs pre-segmentation check.",
                    )
                ]
            ),
        )
    ...
    def from_config(
        ...
        pipe = create_pipe_gen_cpinvoke(
            uid=SBSPreSegcheckInvokeCPModule.uid(),
            spec=spec,
        )
        uow = create_work_unit_gen_index(out_dir=data.storage_path.joinpath("index"))

        return SBSPreSegcheckInvokeCPModule(spec=spec, pipe=pipe, uow=uow)
