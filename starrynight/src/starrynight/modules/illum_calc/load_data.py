...
def gen_illum_calc_load_data(
    ...
    with out_path.joinpath("loaddata.csv").open("w") as f:
        ...
        filename_heads = [f"FileName_Orig{col}" for col in plate_channel_list[0]]
        frame_heads = [f"Frame_Orig{col}" for col in plate_channel_list[0]]
        pathname_heads = [f"PathName_Orig{col}" for col in plate_channel_list[0]]
        loaddata_writer.writerow(
            [*metadata_heads, *filename_heads, *frame_heads, *pathname_heads]
        )
        for index in df_pipe_1.to_dicts():
...
def gen_illum_calc_load_data_par(
    index_path: Path | CloudPath, out_path: Path | CloudPath, prefix: str | None
) -> None:
    """Generate load data for illum calc pipeline.
