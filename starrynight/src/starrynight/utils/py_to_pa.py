"""Adapted from https://github.com/simw/pydantic-to-pyarrow/blob/main/src/pydantic_to_pyarrow/schema.py"""
...
def _get_enum_type(field_type: type[Any]) -> pa.DataType:
    is_str = [isinstance(enum_value.value, str) for enum_value in field_type]  # pyright: ignore
    if all(is_str):
        return pa.dictionary(pa.int32(), pa.string())

    is_int = [isinstance(enum_value.value, int) for enum_value in field_type]  # pyright: ignore
    if all(is_int):
        return pa.int64()
...
def _get_pyarrow_type(
    ...
    if get_origin(field_type) in FIELD_TYPES:
        return FIELD_TYPES[get_origin(field_type)](  # pyright: ignore
